...
const MockLiveTest = () => {
  ...
  useEffect(() => {
    ...
    if (sectionName === "english language") {
      ...
      setDisplayLanguage(selectedLanguage); // fallback to selectedLanguage
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (existingResult) {
          ...
          // Initialize examDataSubmission immediately for the background watcher
          // We'll calculate it once here with the rehydrated values
  ...
  const handleOptionChange = async (index) => {
    setSelectedOptions((prev) => {
      ...
      const result = Api.post(`results/${user?._id}/${id}`, {
        ...
        dispatch(setResults({
          [id]: {
            status: "paused", // Still in progress/paused
  ...
  useEffect(() => {
    ...
    if (isCompositeMode) {
      // Composite: total duration from examData.duration (minutes)
      const totalDuration = (examData.duration || examData.t_time || 0) * 60;
      // Use resultData.timeTakenInSeconds for consistency
      const timeTakenTotal = resultData?.timeTakenInSeconds || 0;
      const remainingTime = totalDuration - timeTakenTotal;

      if (timeTakenFromDB !== timeTakenTotal || timeminus === 0) {
        settimeTakenFromDB(timeTakenTotal);
  ...
  const updateSectionTime = (explicitStatus = null) => {
    ...
    if (isCompositeMode) {
      const time1 = resultData?.section?.[currentSectionIndex]?.timeTaken || 0;
      const time2 = sectionTimes?.[currentSectionIndex] || 0;
      const timeSpentThisSession = Math.floor(
        (now - currentSectionStartTimeRef.current) / 1000
      );
      actualTimeTaken = time1 + time2 + timeSpentThisSession;

      const totalElapsedSeconds = Math.floor((now - examStartTime) / 1000);
      cumulativeTimeTaken = (resultData?.timeTakenInSeconds ?? 0) + totalElapsedSeconds;
    } else {
      // actualTimeTaken = how many seconds of the group/section time have elapsed
      actualTimeTaken = Math.max(0, totalTimeInSeconds - timeminus);
  ...
  useEffect(() => {
    ...
    if (showModal && popupType === "timeExpired" && !isDescriptive) {
      ...
      autoSubmitTimer = setTimeout(async () => {
        ...
        // isSubmitted is already handled inside finishTestAndOpenResult calls submitExam("completed")
