...
const HomeLivetest = () => {
  ...
  useEffect(() => {
    const fetchEnrolledExams = async () => {
      ...
      try {
        ...
        if (response.data && Array.isArray(response.data.enrolledExams)) {
          const examIds = response.data.enrolledExams.map(item => item.Exam._id);
          console.log("Enrolled exams:", examIds);

          setEnrolledExams(examIds);
        }
      } catch (error) {
  ...
  const handleEnroll = async (test) => {
    ...
    try {
      ...
      // Inside handleEnroll function
      setEnrolledExams(prev => [...prev, test._id]);
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {faqs.map((faq, i) => (
              <div key={i} className="border border-green-200 rounded-lg overflow-hidden transition-all duration-200">
                ...
                <div
                  className={`transition-all duration-300 ease-in-out overflow-hidden ${expandedId === i ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'
                    }`}
                >
