...
const Test = () => {
    ...
    const fetchData = async () => {
      if (user?._id) {
        try {
          ...
          const res = await Api.get(`pdf-exams/getExam/${id}`);
          console.log("res", res);
          ...
          const postResponse = await Api.post(`/PDFresults/${user?._id}/${id}`, transformedData);
  ...
  const updateSectionTime = async () => {
    ...
    if (!isComposite) {
      ...
      finalTimeTaken = time1 + time2 + timeSpentThisSession;
    }
    ...
    if (pauseCount < 1) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          setIsPaused(false);
          setPauseCount(0);
          currentSectionStartTimeRef.current = new Date(); // 👈 Reset to now for accurate resumption
                  ...
                  {groupedSections.map((group, groupIndex) => {
                    ...
                    const handleGroupClick = () => {
                      if (!isActiveGroup && group.sections.length > 0) {
                        ...
                        const newStartingIndex = examData.section
                          .slice(0, firstSecIndex)
                          .reduce((acc, s) => acc + (s.questions?.[selectedLanguage?.toLowerCase()]?.length || 0), 0);
                        setCurrentSectionIndex(firstSecIndex);
                        setClickedQuestionIndex(newStartingIndex);
                        setQuestionTime(0);
                      }
                    };
                    ...
                    {activeGroup.sections.map((sec, secIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-1 text-sm font-medium transition-all border rounded ${isSecActive
                            ...
                            <div key={index} className="p-1 rounded-lg m-2 ">
                              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                                ...
                                <label
                                  ...
                                  dangerouslySetInnerHTML={{
                                    __html: sanitizeHtml(option) || "No option available",
                                  }}
                                />
                              </div>
