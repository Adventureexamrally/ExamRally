...
const PdfExamResultPage = () => {
  ...
  useEffect(() => {
    ...
    Api.get(`PDFresults/getresultByExam/${id}`)
      .then(res => {
        ...
        if (user && res.data.length > 0) {
          ...
          if (currentUserResult) {
            ...
            const calculatedPercentile = (100 - (((myRank || 0) / (overallRank || 1)) * 100)).toFixed(2);
        ...
        if (user && res.data.length > 0) {
          ...
          if (currentUserResult?.section) {
            currentUserResult.section.forEach((currentSection, sectionIndex) => {
              ...
              const percentile = (100 - (((rank || 0) / (sortedScores.length || 1)) * 100)).toFixed(2);
            ...
            <tbody className="bg-white divide-y divide-green-100">
              ...
              <tr className="">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-900">
                  {totalAnswered} / {(totalAnswered * (section[0]?.plus_mark || 0)).toFixed(0)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-900">
                  {totalNotAnswered} / 0
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-900">
                  {totalCorrect} / {(totalCorrect * (section[0]?.plus_mark || 0)).toFixed(2)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-900">
                  {totalWrong} / -{(totalWrong * (section[0]?.minus_mark || 0)).toFixed(2)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-900">
                  {(overallScore ?? 0).toFixed(2)} / -{(totalWrong * (section[0]?.minus_mark || 0)).toFixed(2)}
                </td>
              </tr>
            </tbody>
          </table>
...
export default PdfExamResultPage;
