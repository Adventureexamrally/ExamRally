...
const PdfCoupon = ({ data, setshowmodel }) => {
  ...
  const applyCoupon = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.valid) {
        const base = Number(data.offer_price) || Number(data.discountedPrice) || Number(data.price) || Number(data.originalPrice) || 0;
        setDiscountPercent(res.data.discountPercent);
        setFinalPrice(Math.round(Math.max(0, base - (base * res.data.discountPercent) / 100)));
  ...
  const calculateFinalPrice = useCallback(() => {
    let base = Number(data.offer_price) || Number(data.discountedPrice) || Number(data.price) || Number(data.originalPrice) || 0;
    ...
    return { finalValue: Math.round(base - coinDiscount), coinDiscount: Math.round(coinDiscount), priceBeforeCoins: Math.round(base) };
  ...
  useEffect(() => {
    setFinalPrice(calculateFinalPrice().finalValue);
  }, [calculateFinalPrice]);
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      const orderRes = await Api.post("/orders/orders", {
        ...
        notes: { userId: user?._id, email: user?.email, phoneNumber: user?.phoneNumber, courseId: data?._id || data?.months, courseName: data?.title || data?.Title || data?.name, couponCode: discountPercent > 0 ? couponCode : null, isPdf: true },
      });
      ...
      const rzp = new window.Razorpay({
        ...
        currency: "INR",
        name: "Exam Rally",
        description: `PDF Access: ${data?.title || data?.Title || data?.name || "PDF Course"}`,
        ...
        handler: async (response) => {
          try {
            await Api.post("/orders/verify-payment", {
              ...
              coupon: discountPercent > 0 ? couponCode : null,
              amount: displayFinalPrice, isPdf: true,
            });
            await refreshUser();
            toast.success("Payment successful. PDF access granted.");
      ...
      rzp.on("payment.failed", async (r) => {
        ...
        try {
          await Api.post("/orders/payment-failed", { userId: user._id, courseId: data?._id || data?.months, orderId: r.error.metadata?.order_id, paymentId: r.error.metadata?.payment_id, reason: r.error.description || "Payment failed", amount: displayFinalPrice });
        } catch { /* silent */ }
      });
      rzp.open();
    } catch (error) {
  ...
  }, [isSignedIn, user, data, navigate, refreshUser, setshowmodel, couponCode, discountPercent, useCoins, displayPriceBeforeCoins, displayFinalPrice, displayCoinDiscount, loadRazorpayScript]);

  if (!data) return null;

  // Derived values
  const basePrice  = Number(data.price || data.originalPrice || 0);
  const offerPrice = Number(data.offer_price || data.discountedPrice || 0);
  const hasOffer   = offerPrice > 0 && offerPrice !== basePrice;
  const discTag    = hasOffer ? Math.round((1 - offerPrice / basePrice) * 100) : 0;
  const displayedBase = hasOffer ? offerPrice : basePrice;
  const productName = data?.title || data?.Title || data?.name || "PDF Study Material";
      ...
      >
        ...
        >
          ...
          <div style={{ background: "linear-gradient(135deg, #1a3a5c 0%, #0d2340 100%)", padding: "20px 24px 18px", position: "relative", flexShrink: 0 }}>
            <button
              onClick={() => setshowmodel(false)} disabled={isProcessing}
              style={{
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 600, color: "rgba(255,255,255,0.65)", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", padding: "2px 8px", borderRadius: 4 }}>
                <span style={{ display: "flex" }}><PdfIcon /></span>
                Premium Study PDF
              </span>
            </div>
              ...
              <div style={{ marginBottom: 16, border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
                ...
                {!useCoins && (
                  <p style={{ margin: "0 16px 12px", fontSize: 11, color: "#94a3b8", fontWeight: 500 }}>
                    Save up to ₹{Math.round(displayedBase * (settings.maxCoinPaymentPercentage / 100))} using your coins
                  </p>
                )}
              </div>
            )}
            ...
            <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden", marginBottom: 16 }}>
              ...
              <div style={{ padding: "12px 16px" }}>
                {[
                  ...
                  hasOffer && { label: "Offer Discount", value: `-₹${(basePrice - offerPrice).toLocaleString()}`, accent: "#15803d" },
                  discountPercent > 0 && { label: `Coupon (${discountPercent}%)`, value: `-₹${Math.round(displayedBase * discountPercent / 100).toLocaleString()}`, accent: "#15803d" },
