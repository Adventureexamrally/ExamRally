...
const PdfCoupon = ({ data, setshowmodel }) => {
  ...
  const [finalPrice, setFinalPrice] = useState(data.discountedPrice    || 1998);
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (res.data.valid) {
        const baseAmount = Number(data.discountedPrice ) || 0;
        ...
        <div className="p-6 overflow-y-auto flex-grow">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="font-medium">{data?.months} Months</p>
            </div>
            <div className="text-right">
              <p className="text-gray-600">Original Price</p>
              <p className="text-lg font-bold line-through">₹{data.originalPrice}</p>
            </div>
          </div>
          ...
          <div className="bg-green-50 border-l-4 border-green-400 p-4 mb-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-green-700">₹{data.discountedPrice}</p>
              </div>
              <span className="bg-green-100 text-green-800 px-2 py-1 rounded text-sm">
                {Math.round((1 - data.discountedPrice/data.originalPrice) * 100)}% OFF
              </span>
            </div>
          </div>
