...
const Test = () => {
  ...
  useEffect(() => {
    Api.get("exams/getExam/67ac50d70a6d138318229387")
      .then((res) => {
        if (res.data) {
      ...
      <div className="row">
            ...
            <>
              ...
              {examData ? (
                <div>
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <h2>
                      {examData.exam_name} - {examData.test_name}
                    </h2>
                    <p>
                      Marks{" "}
                      <span className="text-white bg-green-400 rounded p-1">
                        + {examData.section[0].plus_mark}
                      </span>
                      &nbsp;
                      <span className="text-white bg-red-500 rounded p-1">
                        - {examData.section[0].minus_mark}
                      </span>
                    </p>
                  </div>
                  <p>
                  ...
                  <p>
                    <strong>Difficulty Level:</strong> {examData.q_level}
                  </p>
                  <p>
        ...
        >
          <div className="badge bg-blue-300 fs-6 p-2">
            Time-Left:{" "}
            <span className="badge bg-blue-500">{timeRemaining}</span>
          </div>

          <div className="container mt-3">
            <div
              className="d-flex flex-wrap gap-2 px-3 py-2 text-center"
              style={{
                maxHeight: "400px",
                overflowY: "auto",
                backgroundColor: "rgb(166 220 247)",
              }}
            >
              {examData?.section?.[currentSectionIndex]?.questions.map(
                (_, index) => {
                  ...
                  const startingIndex = examData.section
                    ...
                    .reduce(
                      ...
                      0
                    ); // Sum up the number of questions in previous sections

                  const currentQuestionIndex = startingIndex + index; // Add the current index within the section to the starting index

                  return (
                    <span
                      key={currentQuestionIndex} // Use the calculated index as the key
                      onClick={() => {
                        handleQuestionClick(currentQuestionIndex); // Use the full question index
                        dispatch(markVisited(currentQuestionIndex)); // Mark the question as visited
                      }}
          ...
          ${
            markedForReview.includes(currentQuestionIndex)
              ? "reviewed smmarkedImg"
              : ""
          }`}
                    >
                      {currentQuestionIndex + 1}{" "}
                      {/* Display the updated question number */}
                    </span>
                  );
                }
              )}
      ...
      <div className="fixed-bottom bg-white p-3">
        <div className="d-flex justify-content-between align-items-center mt-2 flex-column flex-sm-row">
          <button
            ...
            className="btn bg-blue-300 hover:bg-blue-500 fw-bold mb-2 mb-sm-0 w-sm-auto"
          >
            Clear Response
          </button>
          ...
          <button
            ...
            className="btn bg-blue-300 fw-bold hover:bg-blue-500 mb-2 mb-sm-0 w-sm-auto"
          >
            Mark for Review
          </button>
          ...
          <button
            ...
            className="btn bg-blue-500 text-white hover:bg-blue-400 fw-bold mb-2 mb-sm-0 w-50 w-sm-auto"
          >
            Save & Next
          </button>
          ...
          <div className="pl-2 w-sm-auto col">
            <button
              className="btn bg-blue-500 hover:bg-blue-400 text-white w-50 font fw-bold ml-16"
