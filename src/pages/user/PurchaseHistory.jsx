...
<div className="flex flex-col md:flex-row">
  ...
  const formattedDate = new Date(course?.purchaseDate).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
  ...
  return (
    <div key={index} className="w-full">
      <div className="bg-white shadow-md rounded-lg h-full p-4 border-2 border-green-500">
        ...
        <h5 className="text-lg font-semibold mb-2">{course?.courseName}</h5>
        <i className="bi bi-bag-check-fill h3 text-green-500"></i>
        </div>
       
        <h5 className="text-gray-700 mb-1">Amount: {course?.amount}</h5>
