...
import { XMarkIcon, ChevronDownIcon, ChevronUpIcon, GiftIcon, CreditCardIcon } from '@heroicons/react/24/outline';
...
const Coupon = ({ data, setshowmodel }) => {
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (res.data.valid) {
        ...
        setMessage({
          text: `Awesome! ${res.data.discountPercent}% OFF applied`,
          type: 'success'
        });
      } else {
  ...
  const calculateFinalPrice = () => {
    let baseAmount = Number(data.discountedAmount) || Number(data.amount) || 0;
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (discountPercent > 0) {
        priceBeforeCoins = Math.round(priceBeforeCoins - (priceBeforeCoins * discountPercent) / 100);
      }
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            ...
            await refreshUser();
            toast.success('🎉 Welcome aboard! Access granted.');
          ...
          <div className="relative h-40 flex-shrink-0">
            ...
            <div className="absolute bottom-6 left-6 right-6 text-white text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                <CreditCardIcon className="h-4 w-4 text-green-300" />
            ...
            <div className="flex items-end justify-between mb-8 pb-6 border-b border-slate-100">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-2xl font-bold ${data?.discountedAmount ? "text-slate-300 line-through decoration-indigo-500/30" : "text-slate-900"}`}>
                    ₹{data?.amount || 0}
                  </span>
              ...
              <div className="mb-8 overflow-hidden rounded-2xl border border-blue-100/50 bg-gradient-to-br from-blue-50/50 to-white p-5 shadow-sm">
                ...
                {!useCoins && user.coins > 0 && (
                  <p className="text-[10px] text-blue-400 mt-3 font-bold uppercase tracking-widest text-center">
                    Instantly save up to ₹{Math.round((Number(data.discountedAmount) || Number(data.amount) || 0) * (settings.maxCoinPaymentPercentage / 100))} by using coins
                  </p>
                )}
              </div>
            )}
