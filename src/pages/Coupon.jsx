...
const Coupon = ({ data, setshowmodel }) => {
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      const res = await Api.post('/coupons/validate', { couponCode });
  
      if (res.data.valid) {
        const discount = (data.discountedAmount * res.data.discountPercent) / 100;
        const newTotal = data.discountedAmount - discount;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
        <div className="relative">
          ...
          <div className="bg-gradient-to-r from-green-600 to-green-800 p-6 text-white">
            ...
            <p className="opacity-90">Get full access to all course materials</p>
          </div>
        </div>
